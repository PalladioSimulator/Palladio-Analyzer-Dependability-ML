...
public class ProbabilisticSensitivityModel extends SensitivityModel {
    ...
    public double getSensitivityValuesOf(SensitivityProperty property) {
        ...
        var param = probabilisticModel.getDistribution()
            .getParams()
            .get(0)
            .getRepresentation();
    ...
    public double conditionalSensitivity(List<SensitivityProperty> properties) {
        var dist = findMLRandomVariable().getDescriptiveModel()
            .getDistribution();
        var paramRep = (TabularCPD) dist.getParams()
            .get(0)
            .getRepresentation();
    ...
    private boolean isInferenceRequired(List<InputValue<CategoricalValue>> inputs) {
        return (bayesianNetwork.getGroundVariables()
            .size()) > inputs.size();
    }
    ...
    private Optional<Double> retrieveSensitivityValueOf(SensitivityProperty property, SimpleParameter param) {
        var values = param.getValue()
            .split(DefaultParameterParser.SAMPLE_DELIMITER);
    ...
    private Function<String, String[]> parseValues() {
        return value -> {
            return new StringBuilder(value).deleteCharAt(0)
    ...
    private Set<ProbabilityDistribution> retrieveDistributions() {
        return probSensitivityModel.getLocalModels()
            .stream()
    ...
    private SensitivityProperty getGlobalProperty(Map<SensitivityProperty, Double> sensitivityValues) {
        return sensitivityValues.keySet()
            .iterator()
            .next();
    }

    private GroundRandomVariable findRandomVariableFor(SensitivityProperty property) {
        return probSensitivityModel.getLocalProbabilisticModels()
            .stream()
            .flatMap(each -> each.getGroundRandomVariables()
                .stream())
            .filter(variableOf(property))
            .findFirst()
            .orElseThrow(MLSensitivityAnalysisException
                .supplierWithMessage(String.format("There is no variable for measured property %s", property.getId())));
    }
    ...
    private Predicate<GroundRandomVariable> variablesWithParents() {
        return v -> v.getDependenceStructure()
            .isEmpty() == false;
    }
